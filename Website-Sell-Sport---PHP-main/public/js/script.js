...
function getCartAjax(){
	var cart = null;
	/*$('#cartform').hide();
	$('#myCart #exampleModalLabel').text("Giá» hĂ ng");*/
		...
		if(cart.item_count != 0){
			...
			jQuery.each(cart.items,function(i,item){
				...
				if ( typeof(formatMoney) != 'undefined' ){
					div.find('.cart_item_price').html(Haravan.formatMoney(item.price, "")+"â‚«");
					div.find('.total_cart .total_price').html(Haravan.formatMoney(total_price, "")+"â‚«");
				}else {
					div.find('.cart_item_price').html(Haravan.formatMoney(item.price, "")+"â‚«");
					div.find('.total_cart .total_price').html(Haravan.formatMoney(total_price, "")+"â‚«");
				}
			...
			$('.cart_header').find('.cart_price').html(Haravan.formatMoney(cart.total_price,'')+ "â‚«");
			$('.item-total').html(Haravan.formatMoney(cart.total_price,'')+ "â‚«");
			if(cart.item_count == 0){
				$('#cart_popup #cartform').addClass('hidden');
				$('#cart_popup #exampleModalLabel').html('KhĂ´ng cĂ³ sáº£n pháº©m nĂ o trong giá» hĂ ng cá»§a báº¡n.<p class="text-center">Tiáº¿p tá»¥c mua hĂ ng <a href="/collections/all">táº¡i Ä‘Ă¢y</a>.</p>');
			}
			else{
				$('#cart_popup #exampleModalLabel').html('<img src="//hstatic.net/244/1000030244/10/2016/5-31/icon-cart.png" class="icon-cart"/> Giá» hĂ ng cá»§a báº¡n (' + cart.item_count + ' sáº£n pháº©m) <i class="fa fa-play" aria-hidden="true"></i>');
			...
			jQuery('.cart_header_top_box').append('<div class="total_cart"><span>Tá»•ng tiá»n: </span><span class="total_price pull-right">'+Haravan.formatMoney(cart.total_price,'')+ "â‚«"+'</span></div>');
			jQuery('.cart_header_top_box').append('<div class="cart-buttons clearfix"><a href="/cart" class="btn-cart">Xem giá» hĂ ng</a><a href="/checkout" class="btn-check-out">Thanh toĂ¡n</a></div>');
			jQuery('.cart_header').find('.box_text .cart_header_count span').html('('+cart.item_count+')');
			jQuery('.cart_header').find('.box_text .cart_price').html(Haravan.formatMoney(cart.total_price,'')+ "â‚«");
		}
		else{
			/*$('#cart_popup #exampleModalLabel').html('Giá» hĂ ng cá»§a báº¡n Ä‘ang trá»‘ng. Má»i báº¡n tiáº¿p tá»¥c mua hĂ ng.');
			$('#cartform').hide();*/
			jQuery('.cart_header').find('.box_text .cart_header_count span').html('('+cart.item_count+')');
			jQuery('.cart_header').find('.box_text .cart_price').html(Haravan.formatMoney(cart.total_price,'')+ "â‚«");
			jQuery('.cart_header_top_box').html('<div class="cart_empty">Giá» hĂ ng cá»§a báº¡n váº«n chÆ°a cĂ³ sáº£n pháº©m nĂ o.</div>');
		}
	});
}
...
function plus_quantity(btn,variant_id){
	...
	$(btn).parents('.line-item').find('.item-price').html(Haravan.formatMoney(new_price,'') + 'â‚«');
	jQuery.ajax({
		type: 'POST',
		url: '/cart/change.js',
		dataType: 'json',
