...
class login
{
    ...
    function tk_thongbao(){
     ...
     function tk_dttuan(){
         $query = "SELECT SUM(TongTien) as Count FROM HoaDon WHERE YEARWEEK(NgayLap) = YEARWEEK(NOW())And TrangThai = 1 ";
    ...
    function tk_hdtuan(){
         $query = "SELECT count(MaHD) as Count FROM HoaDon WHERE YEARWEEK(NgayLap) = YEARWEEK(NOW())And TrangThai = 1 ";

        return $this->conn->query($query)->fetch_assoc();
    }
    function tk_hdthang($m){
        $query = "SELECT count(MaHD) as Count FROM HoaDon WHERE MONTH(NgayLap) = $m And TrangThai = 1";

        return $this->conn->query($query)->fetch_assoc();
    }
     function tk_hdnam($y){
        $query = "SELECT count(MaHD) as Count FROM HoaDon WHERE YEAR(NgayLap) = $y And TrangThai = 1";
